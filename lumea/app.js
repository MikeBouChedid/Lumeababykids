...
// Restore dev token — localStorage if "stay signed in", sessionStorage otherwise
const _rememberMe = localStorage.getItem('lumea_remember') === '1';
...
function closePage(id) {
  ...
  // Reset forgot password steps when sign in closes
  if (id === 'page-signin') {
    const l = document.getElementById('si-step-login');
    const f = document.getElementById('si-step-forgot');
    const r = document.getElementById('si-step-reset');
...
async function initAuth() {
  ...
  const token    = remember
    ? localStorage.getItem('lumea_dev_token')
    : sessionStorage.getItem('lumea_dev_token');
  if (saved && token) {
...
async function doSignIn() {
  ...
  try {
    ...
    // Store token — in localStorage if remember, sessionStorage if not
    if (data.data.access_token) {
      window._authToken = data.data.access_token;
      if (remember) {
        localStorage.setItem('lumea_dev_token', data.data.access_token);
        sessionStorage.removeItem('lumea_dev_token');
      } else {
        sessionStorage.setItem('lumea_dev_token', data.data.access_token);
        localStorage.removeItem('lumea_dev_token');
      }
    }
    const storage = remember ? localStorage : sessionStorage;
...
function showForgotPassword() {
  document.getElementById('si-step-login').style.display  = 'none';
  document.getElementById('si-step-forgot').style.display = 'block';
  document.getElementById('si-step-reset').style.display  = 'none';
...
function showSignInForm() {
  document.getElementById('si-step-login').style.display  = 'block';
  document.getElementById('si-step-forgot').style.display = 'none';
  document.getElementById('si-step-reset').style.display  = 'none';
}
...
async function sendResetCode() {
  ...
  const btn = document.querySelector('#si-step-forgot .pm-btn');
  btn.textContent = 'Sending…'; btn.disabled = true;
  try {
    ...
    const data = await res.json();
    if (!res.ok) { showToast(data.detail || 'Could not send code'); btn.textContent = 'Send Reset Code →'; btn.disabled = false; return; }
    document.getElementById('fp-email-display').textContent = email;
    document.getElementById('si-step-forgot').style.display = 'none';
    document.getElementById('si-step-reset').style.display  = 'block';
...
async function doResetPassword() {
  ...
  const btn = document.querySelector('#si-step-reset .pm-btn');
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) { showToast(data.detail || 'Reset failed'); btn.textContent = 'Reset Password →'; btn.disabled = false; return; }
